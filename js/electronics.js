// Electronics Projects Page JavaScript
let allProjects = [];
...
async function loadProjects() {
    try {
        ...
        const projectPromises = projectIds.map(async (id) => {
            try {
                const metaResponse = await fetch(`electronics/${id}/metadata.json`);
    ...
    if (projects.length === 0) {
        grid.innerHTML = `
            <div class="empty-state">
                ...
                <p>Check back soon for new electronics projects!</p>
            </div>
        `;
        return;
    }
...
function createProjectCard(project) {
    ...
    card.innerHTML = `
        <div class="project-image">
            ${project.thumbnail ? 
                `<img src="electronics/${project.id}/${project.thumbnail}" alt="${project.title}">` :
                '⚡'
            }
        </div>
